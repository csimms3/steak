...
import { z } from "zod";
import { hiloStart, freshSeeds, type HiloState } from "@/lib/game-engine";
...
export async function POST(req: NextRequest) {
  ...
  if (session?.user?.id) {
    ...
    try {
      const { result, token, seeds, balance } = await withSeeds(userId, 1, async (tx, seeds) => {
        const result = hiloStart(betAmount, { serverSeed: seeds.serverSeed, clientSeed: seeds.clientSeed, nonce: seeds.firstNonce });
  ...
  return NextResponse.json(hiloStart(betAmount, freshSeeds(clientSeed)));
}
